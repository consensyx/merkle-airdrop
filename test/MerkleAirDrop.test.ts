...
describe("Airdrop Merkle Proof Test", () => {
    ...
    beforeEach(async () => {
        ...
        airdrop = await MerkleAirdrop.deploy([token0.address, token1.address, token2.address, token3.address])
    ...
    it("issueTokens + updateMerkleRoot", async function () {
        ...
        await expect(airdrop.connect(user1).issueTokens(id, 0, user1IssueAmount - 1, proof)).to.be.revertedWith(
        ...
        expect(user1AfterBalance.sub(user1BeforeBalance)).to.be.eq(user1IssueAmount)
        ...
        const newMerkleTree = new MerkleTree(elements, keccak256, { hashLeaves: true, sortPairs: true })
        ...
        await airdrop.connect(user2).issueTokens(id, 1, user2IssueAmount, user2Proof)
        await airdrop.connect(user3).issueTokens(id, 2, user3IssueAmount, user3Proof)
    })
    ...
    it("reclaimTokens", async function () {
        ...
        expect(ownerAfterBalance.sub(ownerBeforeBalance)).to.be.eq(remainningTokens)
    })
    ...
    it("updateProject", async function () {
        ...
        await airdrop.updateProject(token0.address, id, newRoot, newProjectName, newAllocated)
